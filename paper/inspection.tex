...
%
Exact GC produces different collected states based on the possible stacks' live addresses:
\begin{align*}
...
\end{lemma}
\begin{proof}
  Soundness follows by induction on the unrolling. Completeness follows
\end{proof}
...
\begin{align*}
  ...
  \widehat{\OK}^*(\mktab,\mperm,\kconsm{\mkframe}{\mpermmap}{\mctx}) &\stepto_2 \setbuild{\passp(\mperm,\mpermmap) \textbf{ and } b}{
          \makont \in \mktab(\mctx),
          \\&\phantom{\stepto_2 \{}b \in \widehat{\OK}(\mktab,\mperm\setminus\mpermmap^{-1}(\Grant),\makont))}
\end{align*}
...
%
We use the accumulator version for the proof for simplicity.
